...
@implementation AMAudioUnitException
...
void AMAudioThrow(OSStatus errCode, const char *functionInfo, int lineNumber) {
    ...
    const char *errString = AMAudioGetErrorStringFromOSStatus(errCode);
    NSString *reason = [NSString stringWithFormat:@"Exception at %s [%d]: %s (OSStatus %ld)", functionInfo, lineNumber, errString, errCode];
    NSException *exception = [NSException exceptionWithName:@"AMAudioUnitException" reason:reason userInfo:nil];
    [exception raise];
}
